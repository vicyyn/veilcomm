...
impl RoutingTable {
    ...
    fn get_lookup_bucket_index(&self, key: &Key) -> usize {
        ...
        let d = Distance::new(&self.node.key, key);
        for i in 0..KEY_LEN {
            for j in (0..8).rev() {
                if (d.0[i] >> (7 - j)) & 0x1 != 0 {
                    return i * 8 + j;
    ...
    pub fn update(&mut self, node: Node) {
        let bucket_idx = self.get_lookup_bucket_index(&node.key);
    ...
    pub fn get_closest_nodes(&self, key: &Key, count: usize) -> Vec<NodeAndDistance> {
        ...
        */
        ...
        for node in &self.kbuckets[bucket_index].nodes {
            ret.push(NodeAndDistance(node.clone(), Distance::new(&node.key, key)));
        }
