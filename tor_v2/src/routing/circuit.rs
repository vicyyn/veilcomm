use crate::{circuit_node::CircuitNode, *};
...
impl OrCircuit {
    ...
    pub fn is_forward(&self, source: SocketAddrV4) -> bool {
        if self.predecessor.socket_address.eq(&source) {
            return true;
        }
        return false;
    }

    pub fn is_backward(&self, source: SocketAddrV4) -> bool {
        return !self.is_forward(source);
    }

    pub fn handle_cell(&self, source: SocketAddrV4, cell: Cell) -> Cell {
...
impl Circuit {
    ...
    pub fn get_cell_destination(&self, source: SocketAddrV4) -> Option<CircuitNode> {
        if let Self::OrCircuit(or_circuit) = self {
